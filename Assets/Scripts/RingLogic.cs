/// @file RingLogic.cs
/// @brief Controls the position and press state of the left hand ring marker,
/// tracking the left hand's position via TwoHandVisualizer and exposing a
/// boolean that is true only while an active forward punch gesture is detected.

using UnityEngine;

/// <summary>
/// Follows the left hand marker point each frame and sets the public down boolean
/// to true only when the left hand is both in a latched press state and still
...
/// Attach to the LeftRing GameObject in the scene.
/// </summary>
public class RingLogic : MonoBehaviour
{
    ...
    /// <summary>
    /// True when the left hand is in a latched press state and its current depth
    ...
    /// <summary>
    /// Called once per frame. Snaps this ring's position to the current left hand
