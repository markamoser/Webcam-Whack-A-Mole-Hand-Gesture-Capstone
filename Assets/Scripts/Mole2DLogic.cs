...
public class Mole2DLogic : MonoBehaviour
{
    ...
    /// state MoleUpdateGameManagerMoleCount: registers the mole with the game manager and enters the rise;
    /// state MolePopUpFromGround: animates the rise with a decelerated scale and positional bob;
    /// state MoleWaitToGetWhackedOrForWaitTimerToExpire: idles and polls for ring hits or timeout;
    /// state MoleDigIntoGroundAndLeave: plays the missed retreat and destroys the mole;
    /// state MoleWhackedAndShowExplosion: holds the whacked flash sprite;
    /// state MoleWhackedAndSquishDown: plays the squash phase of the whacked animation;
    /// state MoleWhackedAndSquishUp: plays the stretch phase of the whacked animation;
    /// state MoleWhackedAndFallIntoGround: plays the final whacked retreat and destroys the mole.
    /// </summary>
    void Update()
    {
